...
private:
    ...
    static constexpr size_t INTERn_N = 3 * ROUNDS_N;
...
public:
    ...
    void generate_r1cs_constraints()
    {
        ...
        for (size_t j = 0; j < ROUNDS_N; ++j)
        {
            ...
            /* y = x^(1/5) <==> y^5 = x; inter[0] = y^4, inter[1] = y^2, inter[2] = y
            */
            // y^4 * y = x
            i[N] += constrain(inter[N][i[N]], inter[N][i[N] + 2], t[N]);
            // y^2 * y^2 = y^4
    ...
    void generate_r1cs_witness()
    {
        ...
        for (size_t j = 0; j < ROUNDS_N; ++j)
        {
            // GTDS
            // y = x^(1/5)
            val(inter[N][i[N] + 2]) = t[N];
            Hash::fifth_inv(val(inter[N][i[N] + 2]));
