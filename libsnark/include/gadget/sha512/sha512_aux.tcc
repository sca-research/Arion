 ...
 *****************************************************************************
 ...
 Implementation of interfaces for auxiliary gadgets for the SHA512 gadget.
...
#ifndef SHA512_AUX_TCC_
#define SHA512_AUX_TCC_
...
namespace libsnark
{
    namespace sha512
    {
        ...
        lastbits_gadget<FieldT>::lastbits_gadget(
            protoboard<FieldT> &pb, const pb_variable<FieldT> &X, const size_t X_bits,
    ...
    */
            ...
            {
                this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>(2 * A, B, A + B - tmp),
                                             FMT(this->annotation_prefix, " tmp"));
                this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>(2 * tmp, C, tmp + C - out),
...
#define SHA512_GADGET_ROTR(A, i, k) A[((i) + (k)) % 64]
        ...
        small_sigma_gadget<FieldT>::small_sigma_gadget(protoboard<FieldT> &pb,
                                                       ...
                                                       const pb_variable<FieldT> &result,
                                                       const size_t rot1, const size_t rot2,
                                                       const size_t shift,
                                                       const std::string &annotation_prefix) :
            gadget<FieldT>(pb, annotation_prefix),
            W(W), result(result)
        {
            ...
            compute_bits.resize(64);
            for (size_t i = 0; i < 64; ++i)
            {
                compute_bits[i].reset(new XOR3_gadget<FieldT>(
                    pb, SHA512_GADGET_ROTR(W, i, rot1), SHA512_GADGET_ROTR(W, i, rot2),
                    (i + shift < 64 ? W[i + shift] : ONE), (i + shift >= 64), result_bits[i],
            ...
            W(W), result(result)
        {
            ...
            for (size_t i = 0; i < 64; ++i)
            {
                compute_bits[i].reset(new XOR3_gadget<FieldT>(
                    pb, SHA512_GADGET_ROTR(W, i, rot1), SHA512_GADGET_ROTR(W, i, rot2),
                    SHA512_GADGET_ROTR(W, i, rot3), false, result_bits[i],
                    FMT(this->annotation_prefix, " compute_bits_%zu", i)));
            }
        ...
        {
            result_bits.allocate(pb, 64, FMT(this->annotation_prefix, " result_bits"));
            pack_result.reset(new packing_gadget<FieldT>(pb, result_bits, result,
        ...
        */
        ...
        void choice_gadget<FieldT>::generate_r1cs_witness()
        {
            for (size_t i = 0; i < 64; ++i)
            {
                this->pb.val(result_bits[i]) = this->pb.lc_val(X[i]) * this->pb.lc_val(Y[i]) +
                                               (FieldT::one() - this->pb.lc_val(X[i])) *
                                                   this->pb.lc_val(Z[i]);
            }
            pack_result->generate_r1cs_witness_from_bits();
        }
        ...
        */
                generate_boolean_r1cs_constraint<FieldT>(
                    this->pb, result_bits[i], FMT(this->annotation_prefix, " result_%zu", i));
                this->pb.add_r1cs_constraint(
                    r1cs_constraint<FieldT>(X[i] + Y[i] + Z[i] - 2 * result_bits[i],
